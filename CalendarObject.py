...
# main container that holds all the dates
# has a list of lists
# each item in the list is a corresponding date calendar_list_container object that is a specific date
# each new date creates new calendar_list_container and all subsequent days on those days will be added
...
class CalendarObject:
    ...
    def __init__(self, CalendarLabel):
        ...
        self.Previous_Button = tk.Button(
            self.root, text='<', command=self.decrement_date_index)
        ...
        self.Next_Button = tk.Button(
            self.root, text='>', command=self.increment_date_index)
    ...
    def add_event(self, event):
        ...
        if len(self.events) == 0:
            calendarObj = calendar_list_container.calendar_date_container(
                event.date, event)

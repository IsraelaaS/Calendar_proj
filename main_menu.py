...
def validate_date_entry(date_str, date_warning_label):
    try:
        ...
        date_warning_label.config(
            text='Invalid Date Format. Please Enter A Time In The Correct Format')
        date_str.set('')
        return False
...
def submit_form(name_string, name_entry_field, name_warning_label, name_label,
    ...
    valid_start_time = validate_time_entry(
        start_time_string, start_time_warning_label)
    valid_end_time = validate_time_entry(
        end_time_string, end_time_warning_label)

    start_end_time_order = compare_time(
        valid_start_time, valid_end_time, start_time_warning_label)
    ...
    if valid_name and valid_date and valid_start_time and valid_end_time and start_end_time_order:
        ...
        current_event = Event(valid_name, valid_date, valid_start_time,
                              valid_end_time, description_string.get())
...
def gui_add_events(label, calendar_visual, add_event_button):
    ...
    start_time_warning_label = tk.Label(
        label, text='', fg='red', bg='lightblue')
    ...
    description_label = tk.Label(
        label, text='Optional Description', bg='lightblue')
